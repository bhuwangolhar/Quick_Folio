...
export default function Certifications({ adminMode = false }: { adminMode?: boolean }) {
  ...
  return (
    <section id="certifications" className="relative py-32 bg-[#080c14] overflow-hidden">
...
function CertCard({
  ...
  return (
    <div
      className={`group relative bg-white/[0.03] border border-white/8 rounded-xl p-6
        hover:border-emerald-400/25 hover:bg-white/[0.06] transition-all duration-500
        ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      style={{ transitionDelay: `${index * 80}ms` }}
    >
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-emerald-400/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
